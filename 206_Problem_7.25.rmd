...
The explanatory variable is ***distance*** and the response variable is ***travel time.***
...
Where R is the correlation between the two variables, and ${ S }_{ x }\quad and \quad{ S }_{ y }$ are the sample
standard deviations of the distance and travel time, respectively.
...
**Solution:**
The slope of the line predicts that it will require `r b1` minutes for each mile travelled.

The intercept of the line b0=`r b0` can be seen as the time when the distnace is zero. It does not make much sense in this context because we cannot travel zero distance in `r b0` minutes, but we could interpret it as the wait time after which we start travelling and caculating the distance travelled and the additional time required per mile.
